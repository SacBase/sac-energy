...
int cyclesStart(void)
{
    struct perf_event_attr pe;
    memset(&pe, 0, sizeof(struct perf_event_attr));

    // Measure actual CPU cycles
    ...
    if (fd == -1) {
        perror("perf_event_open PERF_COUNT_HW_CPU_CYCLES");
        exit(1);
    }
...
unsigned long cyclesStop(int fd)
{
    ...
    unsigned long cycles = read_counter(fd);
